...
class BarChart(BaseChart):
    ...
    def render_parameters(self) -> dict:
        """Renderiza parámetros específicos para bar charts"""
        ...
        with col1:
            ...
            figsize_width = st.slider("Figure Width", 6, 20, 10, key="bar_width")
        
        with col2:
            rotation = st.slider("Label Rotation", 0, 90, 45, key="bar_rotation")
            figsize_height = st.slider("Figure Height", 4, 12, 6, key="bar_height")
        
        with col3:
            mean_col = st.checkbox("Show Overall Mean", key="bar_mean_col")
            
            percentile_low = st.number_input("Lower Percentile", 0.0, 0.5, 0.0, key="bar_perc_low")
            percentile_high = st.number_input("Upper Percentile", 0.5, 1.0, 1.0, key="bar_perc_high")
    ...
    def create_chart(self, df, main_variable: str, extra_variable: str, **params):
        ...
        return plot_financial_barplot(
            df=df,
            numeric_column=main_variable,
            categorical_column=extra_variable,
            **params
        )